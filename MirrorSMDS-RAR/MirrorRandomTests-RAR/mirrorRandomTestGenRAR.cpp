...
*
* FILE: mirrorRandomTestGenRAR.cpp
*
...
* ====		======				===========
* 12/09/16  Alex Reid	 	    Initial implementation and test
*
* DESCRIPTION:
...
* int dimSize			//The randomly generated dimension size of the current 
*						//pattern set
...
* 					Initialize int dimSize, char origPatt[][], char modPatt[][]
* 					For (every row in dimSize){
*						For (every column in dimSize){
...
int main(){
	...
	for(int indx = 1; indx <= testNmbr; indx++){
		...
		snprintf(filename, sizeof(filename), "smtst_%d_in.txt", indx);
		file.open(filename);
		file < "";
		
		int caseNmbr = rand()%15 + 5;
		for(int caseIndx = 0; caseIndx < caseNmbr; caseIndx++){
			int dimSize = rand()%10 + 1;
			char origPatt[dimSize][dimSize];
			char modPatt[dimSize][dimSize];
			for(int rowIndx = 0; rowIndx < dimSize; rowIndx++){
				for(int colmnIndx = 0; colmnIndx < dimSize; colmnIndx++){
					int rndmShad = rand()%2;
					if(rndmShad < 1){
			...
			if(rndmTransform < 1){
				for(int vertIndx = 0; vertIndx < dimSize; vertIndx++){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[dimSize-horixIndx-1][vertIndx];
					}//for
				}//for
			}//if - 90 degrees
				...
				for(int vertIndx = 0; vertIndx < dimSize; vertIndx++){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[dimSize-vertIndx-1][dimSize-horixIndx-1];
					}//for
				}//for
			}//else if - 180 degrees
				...
				for(int vertIndx = 0; vertIndx < dimSize; vertIndx++){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[horixIndx][dimSize-vertIndx-1];
					}//for
				}//for
			}//else if - 270 degrees
				...
				for(int vertIndx = 0; vertIndx < dimSize; vertIndx++){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[dimSize-vertIndx-1][horixIndx];
					}//for
				}//for
				...
				while (vertIndx < dimSize){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[vertIndx][dimSize-horixIndx-1];
					}//for
					vertIndx++;
				}//while
				...
				while (vertIndx < dimSize){
					for(int horixIndx = 0; horixIndx < dimSize; horixIndx++){
						modPatt[vertIndx][horixIndx] = 
								origPatt[dimSize-horixIndx-1][dimSize-vertIndx-1];
					}//for
					vertIndx++;
				}//while
